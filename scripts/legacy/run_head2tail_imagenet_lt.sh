...
# ===============================================================
# Head2Tail proposed method for Imagenet-LT
# ===============================================================
#
# Full pipeline:
#   Step 0 (Optional): LoRA fine-tune SD on Imagenet-LT
...
# ---------------------------------------------------------------
# Uncomment to fine-tune SD on Imagenet-LT first
# This helps the diffusion model understand the Imagenet domain
...
if [ "${FORCE_LORA}" = "1" ] && [ -d "${LORA_DIR}" ]; then
    echo "FORCE_LORA=1, removing existing LoRA directory: ${LORA_DIR}"
    rm -rf "${LORA_DIR}"
fi
...
if [ -n "${RUN_SUFFIX}" ]; then
    BASELINE_TAG="${BASELINE_TAG}_${RUN_SUFFIX}"
fi
...
echo "Force LoRA: ${FORCE_LORA}"
echo "Force generation: ${FORCE_GENERATION}"
...
# ---------------------------------------------------------------
if [ "${FORCE_GENERATION}" = "1" ] && [ -d "${OUTPUT_DIR}" ]; then
    echo "FORCE_GENERATION=1, removing existing generated data directory: ${OUTPUT_DIR}"
    rm -rf "${OUTPUT_DIR}"
fi
